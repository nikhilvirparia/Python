...
class SLList{
    ...
    // find a location within an existing list and APPEND (add after location) a new node containing the input value
    appendValue(loc, value) {
        // your code here
        // step #1) find the location within your existing list (recommend using a counter to find the location)


        var nn = new Node(value) // New Node
        var counter = 1;  // Set the counter = 0
        var runner = this.head; // Addes the runner to head

        // It checkes if the runner.next is not null
        while (runner.next != null) {
            // Stops it reaches the location that's given
            if (counter == loc) {
                // add the value?
                // point our node into the list
                nn.next = runner.next
               // move the head pointer to the correct location
                runner.next = nn
                return this
        ...
        // step #2) using the pointers, sew the new node into your list

    // think about your edge cases! there's a few new ones for this!

    // find a location within an existing list and PREPEND (add before location) a new node containing the input value
...
sll.printValues()
console.log("==========================================")
sll.appendValue(3, 16)
sll.printValues()
// console.log("==========================================")
// sll.prependValue(3, 16)
// sll.printValues()
